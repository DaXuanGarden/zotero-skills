...
| 文件 | 用途 | 要求 |
|---|---|---|
| `{topic_slug}_evidence_review.md` | 人读的证据报告 | 包含 Zotero 检索、PubMed 扩展、主张-证据矩阵、写作改进建议、可点击链接 |
| `{topic_slug}_references.ris` | EndNote 导入文件 | 元数据必须与 Zotero 中检索到的文献保持一致，确保 Word / EndNote 引用准确 |
