...
│
●  ✓ zotero connected
│      /Users/daxuan/.local/bin/zotero-mcp serve
│
└  1 server(s)
```
...
# 每天凌晨 2 点自动更新
0 2 * * * /Users/daxuan/.local/bin/zotero-mcp update-db >> /tmp/zotero-update.log 2>&1

# 或每周更新
0 2 * * 1 /Users/daxuan/.local/bin/zotero-mcp update-db >> /tmp/zotero-update.log 2>&1
```
...
在 OpenCode 终端中**直接对话式提问**即可（打开 OpenCode 后，在 prompt 中输入）：

#### 搜索文献

```
搜索我 Zotero 库中关于 PCOS 和久坐行为的论文，列出前 5 篇
```

```
在 Zotero 中查找 sedentary behavior 相关的文献，给出标题、作者和年份
```

```
搜索 PMID 为 42097137 的文献详细信息
```

#### 语义搜索（概念匹配，非关键词）

```
帮我做语义搜索，找与 "sedentary lifestyle metabolic consequences" 概念相似的文献
```

```
找关于炎症因子与心房颤动关系的研究，用语义搜索
```
...
打开 Zotero 中这篇论文的条目
```
（这会通过 `zotero://select/items/0_GD6Q3CC9` 协议跳转到 Zotero）
...
```bash
# 关键词搜索
zotero-cli search "PCOS sedentary"

# 语义搜索
zotero-cli search --mode semantic "inflammatory cytokines atrial fibrillation"
...
```bash
# Markdown 格式（默认）
zotero-cli get metadata GD6Q3CC9

# BibTeX 格式
zotero-cli g metadata GD6Q3CC9 --format bibtex

# JSON 格式
zotero-cli g metadata GD6Q3CC9 --format json

# 获取全文
zotero-cli get fulltext GD6Q3CC9
```
...
```bash
# 搜索分类
zotero-cli coll search "久坐与PCOS"
...
```bash
# 通过 DOI 添加
zotero-cli add doi 10.1093/humrep/det256
...
open "zotero://select/items/0_GD6Q3CC9"
```
...
Zotero CLI 当前不直接显示分类归属，但可以通过在 OpenCode 中提问获得：
...
> *"在 Zotero 中用语义搜索找到与 'sedentary behavior PCOS metabolic syndrome' 最相关的 5 篇论文，列出每篇的标题、作者、期刊、年份和相似度分数，然后生成 APA 引用，并打开第一篇的 Zotero 条目。"*
...
1. The contribution of diet, physical activity and sedentary behaviour...
   - 作者: Moran LJ 等 | Human Reproduction, 2013 | 相似度: 0.336
   - zotero://select/items/0_GD6Q3CC9

2. Physical activity and sedentary behaviour in women with PCOS...
   - 作者: Tay CT 等 | 2020 | 相似度: 0.275
   - zotero://select/items/0_TVKLYHVL
```
